...
export default function EditOrder() {
    ...
    const orderDetails = order.details.map((detail) => ({
        ...
        product_name: detail.product.nama,
        qty: detail.qty,
        price: detail.price,
    }));
    ...
    const [products, setProducts] = useState(orderDetails)
    ...
    const handleSave = async () => {
        ...
        const response = await UpdateOrder(token, order.id, body);
        if(response.status){
        ...
        >
            ...
            >
                ...
                <DropdownSearchCustomer
                    action={(val) => selectCustomer(val)}
                    initValue={parseInt(data.customer_id)}
                />
                ...
                >
                    ...
                    >
                        <TextInput
                            ...
                            placeholderTextColor="gray"
                            style={styles.input}
                            defaultValue={data.discount.toString()}
                ...
                <Portal>
                    <Dialog style={{borderRadius: 10}} visible={visible} onDismiss={() => setVisible(false)}>
                        ...
                        <Dialog.Actions>
                            ...
                            <Button textColor='white' onPress={() => handleSave()}>Perbarui</Button>
                        </Dialog.Actions>
                    </Dialog>
                </Portal>
            </View>
